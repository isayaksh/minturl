...
interface UrlControllerSpec {
    ...
    fun registerUrl(@CookieValue(name = ConstValue.COOKIE.ACCESS_TOKEN_NAME, required = false) accessToken: String?,
                    @Valid @RequestBody registerUrlReqDto: RegisterUrlReqDto): ResponseEntity<ApiResponse<RegisterUrlResDto>>
    ...
    @Operation(summary = "URL 조회", description = "Shorten URL에 매핑된 실제 URL로 redirection")
    @GetMapping("/{username}/{alias}")
    fun redirectUrl(@PathVariable username: String, @PathVariable alias: String): ResponseEntity<Void>
